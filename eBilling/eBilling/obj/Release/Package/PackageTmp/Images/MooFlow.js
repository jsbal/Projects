...
var MooFlow = new Class({
	...
	createUI: function(){
		...
		var autoPlayCon = new Element('div').addClass('');
		var sliderCon = new Element('span').addClass('');
		var resizeCon = new Element('span').addClass('');		
		if(this.options.useAutoPlay){
			var play = new Element('a').addClass('play')
			play.href = 'http://www.greenerbilling.com/marketing/tour';
		...
		with (Math) {
			this.master.images.each(function(el){
				...
				if(x>-foc*6 && x<foc*6){
					...
					el.con.setStyle('height', H*4 + 'px');
					div.setStyle('width', W*2 + 'px');
					div.setStyle('left', round(((x / z * sz) + sz) - (f * 1) / z * sz) + 'px');
					div.setStyle('top', round(oW * 0.3 - H) + 'px');
